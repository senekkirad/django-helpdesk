    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if request.FILES.get('image'):
                message = Message(ticket=ticket, message=ticket.description, published_by='user', published_at=now(), image=request.FILES.get('image'))
                message.save()
            else:
    ...
    if request.method == 'POST':
        ...
        if request.user.is_staff:
            if request.FILES.get('image2'):
                ticket_msgs = Message.objects.create(ticket=ticket, message=ticket_messages, published_by='agent', published_at=now(), image=request.FILES.get('image2'))
                ticket_msgs.save()
            else:
            ...
            if request.FILES.get('image2'):
                ticket_msgs = Message.objects.create(ticket=ticket, message=ticket_messages, published_by='user', published_at=now(), image=request.FILES.get('image2'))
                ticket_msgs.save()
            else:
